...
class Post:
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"  {self.content}\n"
            f"  ❤ {len(self.likes)} | 💬 {len(self.comments)}"
        )
...
class User:
    ...
    def follow(self, other: "User") -> None:
        ...
        if other in self.following:
            print(f"Você já segue @{other.username}.")
            return
    ...
    def unfollow(self, other: "User") -> None:
        if other in self.following:
            ...
            print(f"Você não segue @{other.username}.")
    ...
    def __str__(self) -> str:
        return f"@{self.username} — {len(self.followers)} seguidores, {len(self.following)} seguindo"
...
class SocialNetwork:
    ...
    def comment_post(self, post_id: int, content: str) -> None:
        if not self.current_user:
            print("Faça login para comentar.")
            return
        post = self._find_post(post_id)
        if not post:
            print("Post não encontrado.")
        ...
        print(f"Comentário adicionado no post #{post.id}.")
    ...
    def follow(self, username: str) -> None:
        if not self.current_user:
            print("Faça login para seguir alguém.")
            return
        user = self.users.get(username)
        if not user:
            print("Usuário não encontrado.")
    ...
    def unfollow(self, username: str) -> None:
        if not self.current_user:
            print("Faça login para deixar de seguir alguém.")
            return
        user = self.users.get(username)
        if not user:
            print("Usuário não encontrado.")
    ...
    def show_feed(self) -> None:
        if not self.current_user:
            print("Faça login para ver seu feed.")
            return
        posts = self.current_user.timeline()
        if not posts:
            print("Seu feed está vazio.")
            return
        for p in posts:
            print("-" * 40)
            print(p)
